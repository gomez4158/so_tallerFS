...
{	
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int inodePerBlock = block_size/ sizeof(Ext2FSInode);

	Ext2FSInode * inode  = new Ext2FSInode;
	unsigned int indexBlockGroupInodo = blockgroup_for_inode(inode_number);
	Ext2FSBlockGroupDescriptor * descriptor = block_group(indexBlockGroupInodo);
	unsigned int inodeTableStart = descriptor -> inode_table;	


	unsigned int offsetBlockGroup  = blockgroup_inode_index(inode_number);
	unsigned int inode_table_offset = offsetBlockGroup/inodePerBlock;
	unsigned int bloqueACargar = inode_table_offset + inodeTableStart;

	char buffer[block_size];

	read_block(bloqueACargar, (unsigned char *)buffer);
	
	unsigned int indiceInodoEnBlock = inode_table_offset % inodePerBlock;

	memcpy(inode, buffer + indiceInodoEnBlock * sizeof(Ext2FSInode),  sizeof(Ext2FSInode));
...
{
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int count_Dir_Block = block_size / sizeof(unsigned int );
	unsigned int blockAux;
 	unsigned int blockAuxSegNivel;
 	unsigned int blockAuxTerNivel;
 	int indiceBloque;
 	int indiceBloqueSegNivel;
 	int indiceBloqueTerNivel;

 if (block_number < 12) // direccionamiento directo
 	{
 		return inode->block[block_number]; 
 	}
 else
 	{
 		if(block_number< 12 + count_Dir_Block) // direccionamiento indirecto
 			{	
 				block_number -= 12;
 				unsigned int buffer[block_size/sizeof(unsigned int) ];
 				blockAux = inode->block[12];
 				read_block(blockAux, (unsigned char *)buffer);
 				return  buffer[block_number];
 			}
 		else 
 		{
 			if (block_number< 12 + count_Dir_Block + count_Dir_Block*count_Dir_Block) // direccionamiento doble
 			{
 				block_number -= 12 + count_Dir_Block;
 				unsigned int buffer[block_size/sizeof(unsigned int) ];
 				indiceBloque = block_number / count_Dir_Block;
 				indiceBloqueSegNivel = indiceBloque % count_Dir_Block;
 				blockAux = inode->block[13];
 				read_block(blockAux, (unsigned char *)buffer);

 				unsigned int bloqueSegNivel = buffer[indiceBloque];
 				read_block(bloqueSegNivel, (unsigned char *)buffer);
 				return buffer[indiceBloqueSegNivel];

 			}
 			else // direccionamiento triple
 			{
 				
					block_number -= 12 + count_Dir_Block + count_Dir_Block* count_Dir_Block;
					unsigned int buffer[block_size/sizeof(unsigned int) ];
	 				indiceBloqueSegNivel = block_number / count_Dir_Block;
	 				indiceBloque = block_number / (count_Dir_Block * count_Dir_Block);
	 				indiceBloqueTerNivel = block_number % count_Dir_Block;	
	 				blockAux = inode->block[13];
 					read_block(blockAux, (unsigned char *)buffer);
 					unsigned int bloqueSegNivel = buffer[indiceBloque];
 					read_block(bloqueSegNivel, (unsigned char *)buffer);
 					unsigned int bloqueTerNivel = buffer[indiceBloqueSegNivel];	
 					read_block(bloqueTerNivel, (unsigned char *)buffer);
 					return buffer[indiceBloqueTerNivel];
...
{	// from inodo directorio.
	...
	int cantidadBloques = from->blocks;
	char buffer[block_size * cantidadBloques];
	
	for (int i = 0; i < cantidadBloques ; ++i)
	{	
		unsigned int bloque = get_block_address(from, i);
		read_block(bloque, (unsigned char *)buffer+i*block_size);		
	}
		
	unsigned int offset = 0;
	while (true)
	{	
		Ext2FSDirEntry * dir = (Ext2FSDirEntry *)((unsigned int)buffer + offset);
		char * strinDir = dir->name;
		int  iguales = strcmp( strinDir, (const char *)filename) ;
		if( iguales) 
			{
